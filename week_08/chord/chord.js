...
d3.csv("../degrees.csv").then((data) => {
  ...
  let arr1 = d3.map(data, d => d.source), //building array of genders
    arr2 = d3.map(data, d => d.target); //building array of degrees

    console.log(arr1, arr2)
  let names = arr1.concat(arr2); //combining the two length 8 arrays into a length 16
  console.log(names)

  names = [...new Set(names)]; //spread syntax, getting unique values in array
  console.log(names)

  // This is an alternative way to do the same thing, but hard to read
  // let names = [
  ...
  const arc = d3.arc()  //building a circle
    .innerRadius(innerRadius)
    .outerRadius(outerRadius);

  // const ribbon = d3.ribbonArrow()
  const ribbon = d3.ribbon()  //building the interior ribbons
