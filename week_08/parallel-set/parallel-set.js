...
d3.csv("../degrees.csv").then((data) => {
  ...
  let graph = nodeLinkData(data, ["source", "target"]); //creating links and nodes for data
  ...
  svg.append("g") // building up the rectangles on the sides
    .selectAll("rect")
    .data(nodes) // using sankey nodes
    .join("rect")
    .attr("x", d => d.x0)
    .attr("y", d => d.y0)
  ...
  svg.append("g") // building ribbons
    ...
    .on("mouseover", function () {
      d3.select(this)
        .attr("opacity", 1); //end of ribbon building
    })
    .on("mouseout", function () {
      d3.select(this)
        .attr("opacity", 0.75); // default opacity
    })
    .on("click", function (e, d) { //click event e = event, d = data
